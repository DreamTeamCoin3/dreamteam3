...
#include "zdreamteam3controldialog.h"
#include "spork.h"
                                                          ...
                                                          currentBalance(-1)
{
    ...
    // "Spending 999999 zDT3 ought to be enough for anybody." - Bill Gates, 2017
    ui->zDT3payAmount->setValidator( new QDoubleValidator(0.0, 21000000.0, 20, this) );
    ...
    ui->labelzDT3SyncStatus->setText("(" + tr("out of sync") + ")");
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        ui->pushButtonMintzDT3->setEnabled(false);
        ui->pushButtonMintzDT3->setToolTip(tr("zDT3 is currently disabled due to maintenance."));

        ui->pushButtonSpendzDT3->setEnabled(false);
        ui->pushButtonSpendzDT3->setToolTip(tr("zDT3 is currently disabled due to maintenance."));
    }
}
...
void PrivacyDialog::on_addressBookButton_clicked()
{
    ...
    if (dlg.exec()) {
        ...
        ui->zDT3payAmount->setFocus();
    }
}

void PrivacyDialog::on_pushButtonMintzDT3_clicked()
{
    if (!walletModel || !walletModel->getOptionsModel())
        return;
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDT3 is currently undergoing maintenance."), QMessageBox::Ok,
                                 QMessageBox::Ok);
        return;
    }
    ...
    ui->TEMintStatus->setPlainText(tr("Minting ") + ui->labelMintAmountValue->text() + " zDT3...");
    ...
    QString strStatsHeader = tr("Successfully minted ") + ui->labelMintAmountValue->text() + tr(" zDT3 in ") +
...
void PrivacyDialog::on_pushButtonSpendzDT3_clicked()
{
    ...
    if(GetAdjustedTime() > GetSporkValue(SPORK_16_ZEROCOIN_MAINTENANCE_MODE)) {
        QMessageBox::information(this, tr("Mint Zerocoin"),
                                 tr("zDT3 is currently undergoing maintenance."), QMessageBox::Ok, QMessageBox::Ok);
        return;
    }
    ...
    if (encStatus == walletModel->Locked || encStatus == walletModel->UnlockedForAnonymizationOnly) {
        ...
        // Wallet is unlocked now, sedn zDT3
        sendzDT3();
        return;
    }
    // Wallet already unlocked or not encrypted at all, send zDT3
    sendzDT3();
}

void PrivacyDialog::on_pushButtonZDreamTeam3Control_clicked()
{
    ZDreamTeam3ControlDialog* zDT3Control = new ZDreamTeam3ControlDialog(this);
    zDT3Control->setModel(walletModel);
    zDT3Control->exec();
}

void PrivacyDialog::setZDreamTeam3ControlLabels(int64_t nAmount, int nQuantity)
{
    ui->labelzDT3Selected_int->setText(QString::number(nAmount));
...
void PrivacyDialog::sendzDT3()
{
    ...
    if(ui->payTo->text().isEmpty()){
        ...
        if (!address.IsValid()) {
            QMessageBox::warning(this, tr("Spend Zerocoin"), tr("Invalid DreamTeam3 Address"), QMessageBox::Ok, QMessageBox::Ok);
            ui->payTo->setFocus();
            return;
        }
    }
    ...
    double dAmount = ui->zDT3payAmount->text().toDouble();
    ...
    // Warn for additional fees if amount is not an integer and change as zDT3 is requested
    ...
    if(!fWholeNumber && fMintChange){
        ...
        strFeeWarning += QString::number(dzFee, 'f', 8) + " DREAMTEAM3 </b>will be added to the standard transaction fees!<br />";
    ...
    QString strAmount = "<b>" + QString::number(dAmount, 'f', 8) + " zDT3</b>";
    ...
    // use mints from zDT3 selector if applicable
    vector<CZerocoinMint> vMintsSelected;
    if (!ZDreamTeam3ControlDialog::listSelectedMints.empty()) {
        vMintsSelected = ZDreamTeam3ControlDialog::GetSelectedMints();
    }
    ...
    // Clear zdreamteam3 selector in case it was used
    ZDreamTeam3ControlDialog::listSelectedMints.clear();
    ...
    for (CZerocoinSpend spend : receipt.GetSpends()) {
        strStats += tr("zDT3 Spend #: ") + QString::number(nCount) + ", ";
    ...
    for (const CTxOut& txout: wtxNew.vout) {
        strStats += tr("value out: ") + FormatMoney(txout.nValue).c_str() + " DreamTeam3, ";
        nValueOut += txout.nValue;

        strStats += tr("address: ");
        CTxDestination dest;
        if(txout.scriptPubKey.IsZerocoinMint())
            strStats += tr("zDT3 Mint");
                               ...
                               const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    for (const auto& denom : libzerocoin::zerocoinDenomList) {
        ...
        strDenomStats = strUnconfirmed + QString::number(mapDenomBalances.at(denom)) + " x " +
                        ...
                        QString::number(nSumPerCoin) + " zDT3 </b>";
    ...
    ui->labelzAvailableAmount->setText(QString::number(zerocoinBalance/COIN) + QString(" zDT3 "));
    ui->labelzAvailableAmount_2->setText(QString::number(matureZerocoinBalance/COIN) + QString(" zDT3 "));
    ui->labelzDT3AmountValue->setText(BitcoinUnits::floorHtmlWithUnit(nDisplayUnit, balance - immatureBalance - nLockedBalance, false, BitcoinUnits::separatorAlways));
}
...
{
    ui->labelzDT3SyncStatus->setVisible(fShow);
}
