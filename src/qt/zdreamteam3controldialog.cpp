...
#include "ui_zdreamteam3controldialog.h"
...
std::list<std::string> ZDreamTeam3ControlDialog::listSelectedMints;
std::list<CZerocoinMint> ZDreamTeam3ControlDialog::listMints;

ZDreamTeam3ControlDialog::ZDreamTeam3ControlDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::ZDreamTeam3ControlDialog),
    model(0)
{
    ui->setupUi(this);
    listMints.clear();
...
ZDreamTeam3ControlDialog::~ZDreamTeam3ControlDialog()
{
    delete ui;
}

void ZDreamTeam3ControlDialog::setModel(WalletModel *model)
{
    this->model = model;
    updateList();
}
...
void ZDreamTeam3ControlDialog::updateList()
{
...
void ZDreamTeam3ControlDialog::updateSelection(QTreeWidgetItem* item, int column)
{
...
void ZDreamTeam3ControlDialog::updateLabels()
{
    int64_t nAmount = 0;
    for (const CZerocoinMint mint : listMints) {
    ...
    ui->labelZDreamTeam3_int->setText(QString::number(nAmount));
    ui->labelQuantity_int->setText(QString::number(listSelectedMints.size()));

    //update PrivacyDialog labels
    privacyDialog->setZDreamTeam3ControlLabels(nAmount, listSelectedMints.size());
}

std::vector<CZerocoinMint> ZDreamTeam3ControlDialog::GetSelectedMints()
{
...
void ZDreamTeam3ControlDialog::ButtonAllClicked()
{
    ui->treeWidget->blockSignals(true);
