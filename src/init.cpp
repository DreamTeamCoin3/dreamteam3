...
void PrepareShutdown()
{
    ...
    RenameThread("dreamteam3-shutoff");
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "dreamteam3.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "dreamteam3d.pid"));
#endif
    strUsage += HelpMessageOpt("-reindex", _("Rebuild block chain index from current blk000??.dat files") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexaccumulators", _("Reindex the accumulator database") + " " + _("on startup"));
    strUsage += HelpMessageOpt("-reindexmoneysupply", _("Reindex the DREAMTEAM3 and zDT3 money supply statistics") + " " + _("on startup"));
    ...
    strUsage += HelpMessageOpt("-port=<port>", strprintf(_("Listen for connections on <port> (default: %u or testnet: %u)"), 17123, 5520));
    ...
    if (GetBoolArg("-help-debug", false))
        strUsage += HelpMessageOpt("-mintxfee=<amt>", strprintf(_("Fees (in DREAMTEAM3/Kb) smaller than this are considered zero fee for transaction creation (default: %s)"),
            FormatMoney(CWallet::minTxFee.GetFeePerK())));
    strUsage += HelpMessageOpt("-paytxfee=<amt>", strprintf(_("Fee (in DREAMTEAM3/kB) to add to transactions you send (default: %s)"), FormatMoney(payTxFee.GetFeePerK())));
    ...
    string debugCategories = "addrman, alert, bench, coindb, db, lock, rand, rpc, selectcoins, tor, mempool, net, proxy, dreamteam3, (obfuscation, swiftx, masternode, mnpayments, mnbudget, zero)"; // Don't translate these and qt below
    if (mode == HMM_BITCOIN_QT)
        debugCategories += ", qt";
    ...
    strUsage += HelpMessageOpt("-minrelaytxfee=<amt>", strprintf(_("Fees (in DREAMTEAM3/Kb) smaller than this are considered zero fee for relaying (default: %s)"), FormatMoney(::minRelayTxFee.GetFeePerK())));
    ...
    strUsage += HelpMessageOpt("-litemode=<n>", strprintf(_("Disable all DreamTeam3 specific functionality (Masternodes, Zerocoin, SwiftX, Budgeting) (0-1, default: %u)"), 0));
    ...
    strUsage += HelpMessageOpt("-masternodeaddr=<n>", strprintf(_("Set external address:port to get to this masternode (example: %s)"), "128.127.106.235:17123"));
    ...
    strUsage += HelpMessageOpt("-backupzdreamteam3=<n>", strprintf(_("Enable automatic wallet backups triggered after each zDT3 minting (0-1, default: %u)"), 1));

//    strUsage += "  -anonymizedreamteam3amount=<n>     " + strprintf(_("Keep N DREAMTEAM3 anonymized (default: %u)"), 0) + "\n";
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           ...
           "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2017-%i The DreamTeam3 Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
{
    RenameThread("dreamteam3-loadblk");
...
/** Sanity checks
 *  Ensure that DreamTeam3 is running in a usable environment with all
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. DreamTeam3 Core is shutting down."));
...
#endif
    // Make sure only a single DreamTeam3 process is using the data directory.
    ...
    if (!lock.timed_lock(boost::get_system_time() + boost::posix_time::seconds(10)))
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. DreamTeam3 Core is probably already running."), strDataDir));
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("DreamTeam3 version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    while (!fLoaded) {
        ...
        do {
            try {
                ...
                // DreamTeam3: load previous sessions sporks if we have them.
                ...
                if (GetBoolArg("-reindexmoneysupply", false)) {
                    if (chainActive.Height() >= Params().Zerocoin_AccumulatorStartHeight()) {
                        RecalculateZDREAMTEAM3Minted();
                        RecalculateZDREAMTEAM3Spent();
                    }
                    RecalculateDREAMTEAM3Supply(1);
                }
                ...
                // DreamTeam3: recalculate Accumulator Checkpoints that failed to database properly
    ...
    if (fDisableWallet) {
        ...
        if (nLoadWalletRet != DB_LOAD_OK) {
            if (nLoadWalletRet == DB_CORRUPT)
                ...
                strErrors << _("Error loading wallet.dat: Wallet requires newer version of DreamTeam3 Core") << "\n";
            else if (nLoadWalletRet == DB_NEED_REWRITE) {
                strErrors << _("Wallet needed to be rewritten: restart DreamTeam3 Core to complete") << "\n";
        ...
        bool fEnableZDreamTeam3Backups = GetBoolArg("-backupzdreamteam3", true);
        pwalletMain->setZDreamTeam3AutoBackups(fEnableZDreamTeam3Backups);
    }  // (!fDisableWallet)
#else  // ENABLE_WALLET
...
//    }
//
//    nAnonymizeDreamTeam3Amount = GetArg("-anonymizedreamteam3amount", 0);
//    if (nAnonymizeDreamTeam3Amount > 999999) nAnonymizeDreamTeam3Amount = 999999;
//    if (nAnonymizeDreamTeam3Amount < 2) nAnonymizeDreamTeam3Amount = 2;
    ...
    LogPrintf("Anonymize DreamTeam3 Amount %d\n", nAnonymizeDreamTeam3Amount);
    ...
    /* Denominations
       ...
       For example:
       1DREAMTEAM3+1000 == (.1DREAMTEAM3+100)*10
       10DREAMTEAM3+10000 == (1DREAMTEAM3+1000)*10
    */
